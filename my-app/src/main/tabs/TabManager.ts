...
export class TabManager {
  ...
  private statusBarView: WebContentsView;
  ...
  constructor(win: BrowserWindow, opts?: { dataDir?: string; partition?: string; guest?: boolean }) {
    ...
    // Status bar — a tiny view at the bottom-left showing the hovered link URL.
    // Separate from the shell view so it can sit at the window bottom without
    // blocking page clicks (the shell view is clipped to chrome height).
    this.statusBarView = new WebContentsView({
      webPreferences: { contextIsolation: true, sandbox: true },
    });
    this.statusBarView.setBackgroundColor('#00000000');
    this.statusBarView.setVisible(false);
    this.win.contentView.addChildView(this.statusBarView);
    this.statusBarView.webContents.loadURL('about:blank').then(() => {
      this.statusBarView.webContents.executeJavaScript(`
        document.documentElement.style.cssText='margin:0;padding:0;background:transparent';
        document.body.style.cssText='margin:0;padding:0;background:transparent;overflow:hidden;display:flex;align-items:flex-end;height:100vh';
        const b=document.createElement('div');
        b.style.cssText='height:${STATUS_BAR_HEIGHT - 1}px;padding:0 8px;display:flex;align-items:center;background:#292a2d;border-top:1px solid #3c3c3c;border-right:1px solid #3c3c3c;border-top-right-radius:4px;width:100%';
        const s=document.createElement('span');
        s.id='u';
        s.style.cssText='font:11px/1 -apple-system,system-ui,sans-serif;color:#9aa0a6;white-space:nowrap;overflow:hidden;text-overflow:ellipsis';
        b.appendChild(s);
  ...
  private positionStatusBar(): void {
    ...
    const [winWidth, winHeight] = this.win.getContentSize();
    const w = Math.min(STATUS_BAR_MAX_WIDTH, Math.floor(winWidth * 0.5));
    const x = this.sidePanelPosition === 'left' ? this.sidePanelWidth : 0;
    this.statusBarView.setBounds({
      x,
      y: winHeight - STATUS_BAR_HEIGHT,
      width: w,
      height: STATUS_BAR_HEIGHT,
    });
  }

  private updateStatusBar(url: string): void {
    if (this.win.isDestroyed()) return;
    if (url) {
      this.statusBarView.webContents
        .executeJavaScript(`document.getElementById('u').textContent=${JSON.stringify(url)}`)
        .catch(() => {});
      this.positionStatusBar();
      this.statusBarView.setVisible(true);
    } else {
      this.statusBarView.setVisible(false);
    }
  }
  ...
  createTab(url?: string, id?: string): string {
    ...
    // Keep shell overlay and status bar on top of all tab views
    this.win.contentView.addChildView(this.shellView);
    this.win.contentView.addChildView(this.statusBarView);
