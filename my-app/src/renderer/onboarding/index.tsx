...
type OnboardingStep = 'welcome' | 'chrome-import' | 'account' | 'complete';
...
function OnboardingApp(): React.ReactElement {
  ...
  function handleWelcomeNext(): void {
    setState((prev) => ({ ...prev, step: 'chrome-import' }));
  }
  ...
  if (step === 'account') {
    return (
      <AccountCreation
        onBack={() => setState((prev) => ({ ...prev, step: 'chrome-import', oauthError: null }))}
  ...
  if (step === 'chrome-import') {
    return (
      <ChromeImport
        onNext={() => setState((prev) => ({ ...prev, step: 'account' }))}
        onSkip={() => setState((prev) => ({ ...prev, step: 'account' }))}
      />
    );
  }
